 /**
 * deze functie zorgt ervoor dat de game wordt gerenderd op een consistent framerate
 * DIT IS HEEL BELANGERIJK WANT ANDERS IS DE SNELHEID VAN DE GAME NIET CONSTANT
 * 
 * een probleem wat ik dus had was dat de refreshrate van mijn monitor 165 was
 * waardoor de game dus 1.65x snelheid runde en dat was NIET chill
 * de game werkte dus maar hij ging veel te snel
 */
...
window.requestAnimFrame = (function(){
  ...
  let GameEngine = function(canvasSelector, scoreSelector) {
		  ...
		  if (typeof(Storage) !== "undefined") {
			  localStorage["board.0.name"] = "Milad Sahar";
			  localStorage["board.0.score"] = 9990;
			  localStorage["board.1.name"] = "Lars Swagerman";
		  ...
		  var Update = function() {
			  ...
			  // ongebruikte objecten verwijderen (want anders stapelen ze langzaam op en dan crasht je chrome)
			  for (var i = 0; i < engine.objects.length; i++) {
  ...
  /**
   * constructor voor objecten die worden gerenderd
  ...
  /**
   * Asteroide constructor
   * @param {Number} rad radius van de asteroide
   */
  var Asteroid = function(rad){
		  ...
		  asteroid.Update = function() {
			  ...
			  // dit zorgt ervoor dat je in het canvas blijft als je eraf probeert te gaan
...
//   ----------------------------------------------------------------
  ...
  var Bullet = function() {
		  ...
		  bul.Update = function () {
			  ...
			  // dubbel check of je niet toevallig een asteroide tegen je gezicht aankrijgt
			  var pos = this.position;
			  var collision = false;
			  game.eachByName("asteroid", function(node, k){
				  // check of je dichtbij genoeg bent
			  ...
			  // als de rots van het canvas afgaat dan wordt ie gwn verwijderd
   ...
   * Burst constructor
   * 
   * Oke dus basically, wanneer je een rots compleet hebt vernietigd (incl zn kids) dan exploderen de kleinste steentjes in tientallen piepkleine stukken
   * die gelukkig geen schade meer aanrichten
   * @param {Object} options
   */
  ...
  /* Helpers */
  /**
   * kruising tussen 2 lijnen checken
  ...
  /**
   * leaderboard box aan het einde van de game (voor als je dood ging)
   * @param {String} selector
   * @param {Number} score
   */
  var EngGameMessage = function (selector, score) {
		  var scores = [];
		  // als de local storage bestaat
		  if (typeof(Storage) !== "undefined") {
			  // ff kijken of we nog scores achterin hebben liggen
			  ...
			  // ff de html klaarmaken om de scores te laten zien
  ...
  /**
   * ok dus wat we hier doen is we maken van de naam die je invult een globale variabele
   * @param {HTML node} node
   */
  var SaveName = function (node) {game.name = node.value;};
  /**
   * oke dus basically dit verandert de kleur van de game maar uhhh, ik vond wit gwn het beste
   * want dat is loyaal aan de originele Asteroids game
   * 
   * @param {Number} color 	kleur codes: 0 - groen, 1 - wit, 2 - blauw.
   */
  ...
  game.color = "";
  if (typeof(Storage) !== "undefined") {
		  game.color = localStorage["game.color"] || "#FFFFFF";
		  var c = 0;
		  switch (game.color) {
			  case "#F00":
				  c = 1;
  ...
  ship.Update = function () {
		  ...
		  // snelheid aanpassen wanneer de motor start
		  if (game.input.forward) {
			  ...
			  // en wegdoen als je W of pijltje omhoog loslaat
			  this.points = [
				  {x: 0, y: 0},
				  {x: 10, y: 10},
				  {x: 0, y: -20},
		  ...
		  // hiermee vertraag je als je niet meer beweegt
		  this.position.x += this.velocity.x;
		  this.position.y += this.velocity.y;
		  this.velocity.x *= this.inertia;
		  this.velocity.y *= this.inertia;
  
		  // teleporteren als je van het canvas afgaat
		  if (this.position.x > game.canvas.width) {
			  this.position.x -= game.canvas.width;
		  }
		  if (this.position.x < 0) {
		  ...
		  // JE BENT TOCH NIET GECRASHED HÉ?
		  var pos = this.position;
  ...
  /* als je de steen kapot schiet worden hier kleinere gemaakt */
  for (var i = 0; i < 4; i++) {
		  var rock = new Asteroid(80);
		  rock.Start();
		  game.objects.push(rock);
